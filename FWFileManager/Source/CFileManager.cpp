...
#ifdef _RELEASE_DEBUG_
#include"CConsole.h"
#endif // _RELEASE_DEBUG_
...
CFileManager::CFileManager(): m_pCurrentFile(nullptr), 
m_pPlatform(nullptr), m_pICTag(nullptr), m_pFileLsn(nullptr)
{
	m_lsRequest.clear();
	m_lsFinished.clear();
}
...
#else
	m_pPlatform = &CWeb::Instance(); 
#endif

	m_pICTag = new CICTarget(EICTAGTYPE::ICT_FILEMAG);
...
{
	return m_pICTag == nullptr ? false : m_pICTag->RegisterLst(pLsn);
}


void CFileManager::UnregisterLst(CListener* pLsn)
{
	m_pICTag->UnregisterLst(pLsn);
}
...
void CFileManager::Monitor()
{
	...
	case CFile::EFSTATE::PARSE_SUC:
	{
		...
		std::string str = "Parse successfully, file name:";
		str += m_pCurrentFile->nameFile();
		str += "\n";

		CConsole::Instance().Output(str.c_str());
...
{
	if ((nullptr == pFile)||(nullptr == m_pICTag))
	{
		return;
	}

	m_pICTag->Start();
	for (; !m_pICTag->isEnd(); m_pICTag->MoveToNext())
	{
		m_pICTag->current()->OnCallback(pFile);
	}
...
bool CFileManager::TransformFileFromFBX2FWD(const char* pszNameFilePathSrc)
{

	if (pszNameFilePathSrc == nullptr)
	{
		return false;
	}

	m_pFileLsn = new CFileOpListener(nullptr);
	if (!m_pFileLsn->Create(pszNameFilePathSrc))
	{
		return false;
	}

	CInterchangeCenter::Instance()->AddApplication(m_pFileLsn);

	CFileDescribe* pFD = new CFileDescribe(EFILETYPE::FI_FBX);
	if (!pFD->Initialize(pszNameFilePathSrc))
	{
		delete pFD;
		return false;
	}
...
bool CFileManager::Package(const char* pszNameFolder, const char* pszNamePackage)
{
	if ((nullptr == m_pPlatform) || (nullptr == pszNameFolder) || (nullptr == pszNamePackage))
	{
		return false;
	}

	CFileNode* pFileTreeRoot = m_pPlatform->CreateFileTree(pszNameFolder);
	if (nullptr == pFileTreeRoot)
	{
		return false;
	}
...
bool CFileManager::Unpackage(const char* pszNamePath, const char* pszNamePackage)
{
	return true;
}
