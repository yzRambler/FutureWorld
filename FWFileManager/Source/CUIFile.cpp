...
CSceneNodeAspect* CUIFile::CreateSceneNodeAspect(int orderTime, cJSON* pJS)
{
	...
	for (cJSON* pJSSceneNodeCom = pJS->child; pJSSceneNodeCom != 0; pJSSceneNodeCom = pJSSceneNodeCom->next)
	{
		...
		//pJSSceneNodeCom = pJSSceneNodeCom->next;
	}
...
CAspect* CUIFile::CreateConstructAspect(CAspect* pHostApt, cJSON* pJS)
{
	...
	for (pJSConstructCom = pJS->child; pJSConstructCom != nullptr; pJSConstructCom = pJSConstructCom->next)
	{
		...
		{
			/*
			if (strcmp(pJSConstructCom->valuestring, "Camera") == 0)
			{
				CCameraAspect* pCamApt = new CCameraAspect(pHostApt, 0);
				pCamApt->Create();

				pAspect = pCamApt;
				pAspect->setTypeApt(EAPTTYPE::CST_CAMERA);
				continue;
			}
			else if (strcmp(pJSConstructCom->valuestring, "Light") == 0)
			{
				CLightAspect* pLightApt = new CLightAspect(pHostApt, 0);
				pLightApt->Create(m_pSceneApt->orderTime());

				pAspect = pLightApt;
				pAspect->setTypeApt(EAPTTYPE::CST_LIGHT);
				continue;
			}
			else if (strcmp(pJSConstructCom->valuestring, "Essety") == 0)
			{
	...
	for (pJSConstructCom = pJS->child; pJSConstructCom != 0; pJSConstructCom = pJSConstructCom->next)
	{
		...
		{
			//pAspect->setName(CloneString(pJSConstructCom->valuestring));
...
bool CUIFile::InitializeCameraAspect(cJSON* pJS, CCameraAspect* pCameraApt)
{
	...
	for (cJSON* pJSConstructCom = pJS; pJSConstructCom != nullptr; pJSConstructCom = pJSConstructCom->next)
	{
		...
		else if (strcmp(pJSConstructCom->string, "ViewPort") == 0)
		{
			...
			float fw = 0.0;
			float fh = 0.0;
			//cJSON* pJSViewPortCom = pJSConstructCom->child;
			//while (pJSViewPortCom)
		...
		else if (strcmp(pJSConstructCom->string, "Enable") == 0)
		{
			...
			while (pJSEnableArrayItem)
			{
				...
				if ((strcmp(pJSEnableArrayItem->valuestring, "Depth") != 0)
					&& (strcmp(pJSEnableArrayItem->valuestring, "Stencil") != 0)
					&& (strcmp(pJSEnableArrayItem->valuestring, "Blend") != 0)
					&& (strcmp(pJSEnableArrayItem->valuestring, "CullFace") != 0))
				{
					return false;
				}
		...
		else if (strcmp(pJSConstructCom->string, "Disable") == 0)
		{
			...
			while (pJSDisableArrayItem)
			{
				...
				if ((strcmp(pJSDisableArrayItem->valuestring, "Depth") != 0)
					&& (strcmp(pJSDisableArrayItem->valuestring, "Stencil") != 0)
					&& (strcmp(pJSDisableArrayItem->valuestring, "Blend") != 0)
					&& (strcmp(pJSDisableArrayItem->valuestring, "CullFace") != 0))
				{
					return false;
				}
		...
		else if ((strcmp(pJSConstructCom->string, "FrontFace")) == 0)
		{
			continue;
		}
		else if ((strcmp(pJSConstructCom->string, "CullFace")) == 0)
		{
			continue;
		}
		else if (strcmp(pJSConstructCom->string, "Clear") == 0)
		{
			...
			while (pJSClearArrayItem)
			{
				...
				if ((strcmp(pJSClearArrayItem->valuestring, "Depth") != 0)
					&& (strcmp(pJSClearArrayItem->valuestring, "Stencil") != 0)
					&& (strcmp(pJSClearArrayItem->valuestring, "Color") != 0))
				{
					return false;
				}
